...
class AuthModel {
    ...
    public function find($id) {
        return db::select("SELECT * FROM `users` WHERE ID=?", array($id));
    }
    ...
    public function register($mail, $user, $pass) {
        ...
        $address = $_SERVER["REQUEST_URI"] . "verifymail/$verify_key";
    ...
    public function forgot($mail) {
        if(db::count("SELECT COUNT(*) FROM `users` WHERE EMAIL=?", array($mail)) > 0) {
            ...
            $address = $_SERVER["REQUEST_URI"] . "login/forgot-password/$token/";
